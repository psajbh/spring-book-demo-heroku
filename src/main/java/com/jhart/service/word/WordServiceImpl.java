...
public class WordServiceImpl implements WordService {
	...
	private Map<Integer, ArrayList<CharDisplay>> wordNamesAnalysis(String words, Integer wordCount) {
		log.info("WordServiceImpl - wordNamesAnalysis - start");
		...
		if (level1CharDisplay.size() > 0 && level2CharDisplay.size() > 0 && level3CharDisplay.size() > 0
				&& level4CharDisplay.size() > 0 && level5CharDisplay.size() > 0) {
	...
	private List<Word> processChar1Unavailable(WordSupportDto wordSupportDto, List<Word> availablewords) {
		//log.info("WordServiceImpl - processChar1Unavailable");
		if (!wordSupportDto.getInChar1().isEmpty()) {
			//log.debug("WordServiceImpl - processChar1Unavailable = processing - " + wordSupportDto.getInChar1());
			for (int i = 0; i < availablewords.size(); i++) {
				Word w = availablewords.get(i);
				if (!w.isUnavailable()) {
			...
			//log.debug("WordServiceImpl - processChar1Unavailable - processing for notInChar1 - values");
					...
					if (!w.isUnavailable()) {
						if (1 == wordSupportDto.getNotInChar1().length()) {
							if (w.getChar1().equals(wordSupportDto.getNotInChar1())) {
								//log.debug("char1 value of '" + w.getChar1() + "' sets the word: " + w.getWord() as unavailable");
								w.setUnavailable(true);
							}
						} else {
							int n = 1;
							...
							for (String s : results) {
								...
								if (w.getChar1().contains(s)) {
									//log.debug("WordServiceImpl - processChar1Unavailable - setting word: " + w									+ " as NOT available as it matches the character " + s);
									w.setUnavailable(true);
									break;
								}
							}
	...
	private List<Word> processChar2Unavailable(WordSupportDto wordSupportDto, List<Word> availablewords) {
		//log.info("WordServiceImpl - processChar2Unavailable");
		if (!wordSupportDto.getInChar2().isEmpty()) {
			...
			//log.debug("WordServiceImpl - processChar2Unavailable - processing for notInChar2 - values");
					...
					if (!w.isUnavailable()) {
						if (1 == wordSupportDto.getNotInChar2().length()) {
							if (w.getChar2().equals(wordSupportDto.getNotInChar2())) {
								//log.debug("char2 value of '" + w.getChar2() + "' sets the word: " + w.getWord() as unavailable");
								w.setUnavailable(true);
							}
						} else {
							int n = 1;
							...
							for (String s : results) {
								if (w.getChar2().contains(s)) {
									//log.debug("WordServiceImpl - processChar2Unavailable - setting word: " + w
									//		+ " as NOT available as it matches the character " + s);
									w.setUnavailable(true);
									break;
								}
							}
	...
	private List<Word> processChar3Unavailable(WordSupportDto wordSupportDto, List<Word> availablewords) {
		//log.info("WordServiceImpl - processChar3Unavailable");
		if (!wordSupportDto.getInChar3().isEmpty()) {
			...
			//log.debug("WordServiceImpl - processChar3Unavailable - processing for notInChar3 - values");
					...
					if (!w.isUnavailable()) {
						...
						if (1 == wordSupportDto.getNotInChar3().length()) {
							if (w.getChar3().equals(wordSupportDto.getNotInChar3())) {
								//log.debug("WordServiceImpl - processChar3Unavailable -char3 value of '" + w.getChar3()
								//		+ "' sets the word: " + w.getWord() + " as unavailable");
								w.setUnavailable(true);
							}
						} else {
							int n = 1;
							...
							for (String s : results) {
								if (w.getChar3().contains(s)) {
									//log.debug("WordServiceImpl - processChar3Unavailable - setting word: " + w
									//		+ " as NOT available as it matches the character " + s);
									w.setUnavailable(true);
									break;
								}
	...
	private List<Word> processChar4Unavailable(WordSupportDto wordSupportDto, List<Word> availablewords) {
		//log.info("WordServiceImpl - processChar4Unavailable");
		if (!wordSupportDto.getInChar4().isEmpty()) {
			...
			//log.debug("WordServiceImpl - processChar4Unavailable - processing for notInChar4 - values");
					...
					if (!w.isUnavailable()) {
						...
						if (1 == wordSupportDto.getNotInChar4().length()) {
							if (w.getChar4().equals(wordSupportDto.getNotInChar4())) {
								//log.debug("WordServiceImpl - processChar4Unavailable - char4 value of '" + w.getChar4()
								//		+ "' sets the word: " + w.getWord() + " as unavailable");
								w.setUnavailable(true);
							}
						} else {
							...
							for (String s : results) {
								if (w.getChar4().contains(s)) {
									//log.debug("WordServiceImpl - processChar4Unavailable - setting word: " + w
									//		+ " as NOT available as it matches the character " + s);
									w.setUnavailable(true);
									break;
								}
	...
	private List<Word> processChar5Unavailable(WordSupportDto wordSupportDto, List<Word> availablewords) {
		//log.info("WordServiceImpl - processChar5Unavailable");
		if (!wordSupportDto.getInChar5().isEmpty()) {
			...
			//log.debug("WordServiceImpl - processChar5Unavailable - processing for notInChar5 - values");
						...
						if (1 == wordSupportDto.getNotInChar5().length()) {
							...
							for (String s : results) {
								if (w.getChar5().contains(s)) {
									//log.debug("WordServiceImpl - processChar5Unavailable - setting word: " + w
									//		+ " as NOT available as it matches the character " + s);
									w.setUnavailable(true);
									break;
								}
	...
	private List<Word> processUnavailable(String unavailableChars, List<Word> originalWords) {
		//log.info("WordServiceImpl - processUnavailable");
	...
	private List<Word> setOriginalWords(WordSupportDto wordSupportDto) {
		log.info("WordServiceImpl - setOriginalWords - start");
		...
		if (null == allWords) {
			...
			log.debug("WordServiceImpl - setOriginalWords - wordHolder size: " + wordHolder.size() + " elapsed time: "
		...
		log.info("WordServiceImpl - originalWords size: " + originalWords.size());
		return originalWords;
	}
